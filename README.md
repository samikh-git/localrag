...
* **Pip** - simply run `pip install perpetua`
* **Homebrew** - run:

    ```bash
    brew tap samikh-git/tools
    brew install perpetua
    ```
...
* `perpetua init`: initializes a localrag project. Creates the .rag directory
...
* `perpetua diff`: shows the difference between the staged files and currently tracked files
...
* `perpetua help`: provides links to documentation.
...
This directory is structured in two subdirectories: 
